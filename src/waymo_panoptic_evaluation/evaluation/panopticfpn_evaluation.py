import os
from pathlib import Path

import torch
from detectron2 import model_zoo
from detectron2.checkpoint import DetectionCheckpointer
from detectron2.config import get_cfg
from detectron2.modeling import build_model
from torch.utils.data import DataLoader
from torchmetrics.detection import PanopticQuality
from detectron2.data import MetadataCatalog

from waymo_panoptic_evaluation import mappings
from waymo_panoptic_evaluation.mappings import (
    WAYMO_STUFF_CLASSES_IDS,
    WAYMO_THING_CLASSES_IDS,
)
from waymo_panoptic_evaluation.waymo_dataset import WaymoDataset


def evaluate_panopticfpn(waymo_data_dir: Path) -> None:
    dataset = WaymoDataset(image_directory=waymo_data_dir)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=1)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    cfg = get_cfg()
    cfg.merge_from_file(
        model_zoo.get_config_file("COCO-PanopticSegmentation/panoptic_fpn_R_50_3x.yaml")
    )
    cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url(
        "COCO-PanopticSegmentation/panoptic_fpn_R_50_3x.yaml"
    )
    cfg.MODEL.DEVICE = device.type
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5
    
    train_dataset = cfg.DATASETS.TRAIN[0]  # Usually something like "coco_2017_train_panoptic_separated"

    # Retrieve metadata
    metadata = MetadataCatalog.get(train_dataset)

    # Access thing and stuff classes
    thing_classes = metadata.thing_classes
    stuff_classes = metadata.stuff_classes

    print("Thing classes:", {name: idx for idx, name in enumerate(thing_classes)})
    print("Stuff classes:", {name: idx for idx, name in enumerate(stuff_classes)})

    model = build_model(cfg).to(device)
    DetectionCheckpointer(model).load(cfg.MODEL.WEIGHTS)

    model.eval()

    pq_metric = PanopticQuality(
        things=WAYMO_THING_CLASSES_IDS,
        stuffs=WAYMO_STUFF_CLASSES_IDS,
        return_sq_and_rq=True,
        return_per_class=False,
    ).to(device)

    with torch.no_grad():
        for image, masks in dataloader:
            image = image.squeeze(0).permute(2, 0, 1).to(device)

            semantic_mask, instance_mask = masks
            semantic_mask = semantic_mask.squeeze(0).to(device)
            instance_mask = instance_mask.squeeze(0).to(device)
            target_panoptic_tensor = torch.stack(
                (semantic_mask.squeeze(0), instance_mask.squeeze(0)),
                dim=-1,
            ).to(torch.long)

            img_height, img_width = image.shape[-2:]
            pred_panoptic_tensor = torch.zeros(
                (img_height, img_width, 2), dtype=torch.long, device=device
            )

            inputs = [{"image": image, "height": img_height, "width": img_width}]
            pred_segmentation_map, segments_info = model(inputs)[0]["panoptic_seg"]

            for segment_info in segments_info:
                segment_id = segment_info["id"]
                category_id = segment_info["category_id"]
                is_thing = segment_info["isthing"]
                instance_id = segment_info["instance_id"] + 1 if is_thing else 0

                waymo_class_id = mappings.get_waymo_class_id_from_coco(
                    category_id, is_thing
                )
                mask = pred_segmentation_map == segment_id

                pred_panoptic_tensor[..., 0][mask] = waymo_class_id
                pred_panoptic_tensor[..., 1][mask] = instance_id

            pq_metric.update(pred_panoptic_tensor, target_panoptic_tensor)
            break

    final_pq_results = pq_metric.compute()
    print(final_pq_results)
    print(f"Panoptic Quality (PQ): {final_pq_results[0].item():.4f}")
    print(f"Segmentation Quality (SQ): {final_pq_results[1].item():.4f}")
    print(f"Recognition Quality (RQ): {final_pq_results[2].item():.4f}")


if __name__ == "__main__":
    evaluate_panopticfpn(
        waymo_data_dir=os.environ.get(
            "WAYMO_DATA_DIR", Path(__file__).parents[3] / "waymo_data"
        )
    )
